...
#' @param overlap overlaps between two adjacent bins (bp)
...
#' ##and overlap of 500 bp between two adjacent bins
#' 
#'  chromFile <- "mm10" 
#' # will be parsed as system.file("extdata", "mm10.chrom.sizes", package = "ChIPseqSpikeFree")
#'  binDF <- GenerateBins(chromFile=chromFile,binSize=1000, overlap=500,
#'  withChr=FALSE,prefix="mm10")
#'
#' ##2. generate a hg19 binFile with chr and use a binSize of 2000 bp 
#' 
#'  chromFile <- "hg19" 
#' # will be parsed as system.file("extdata", "hg19.chrom.sizes", package = "ChIPseqSpikeFree")
#'  binDF<- GenerateBins(chromFile=chromFile,binSize=2000, overlap=0,
#'  withChr=TRUE,prefix="hg19")
#' 
#' 
#' 
GenerateBins <- function(chromFile=input,binSize=1000, overlap=0,withChr=TRUE){
    #generate genome-wide bins for counting purpose

    options(stringsAsFactors = FALSE)
    if (!file.exists(chromFile)){
       chromFile <- system.file("extdata", paste0(chromFile,".chrom.sizes"), package = "ChIPseqSpikeFree")
       if (!file.exists(chromFile)){
...
#' 
#' metaFile <- system.file("extdata", "sample_meta.txt", package = "ChIPseqSpikeFree")
...
ReadMeta <- function(metaFile="sample_meta.txt"){
    ...
    if (! "COLOR" %in% colnames(meta)){
        myColors <- c("red","blue","green","#F4B5BE","#5F5647",
...
#'
ParseReadCounts <- function(data=rawCountDF,metaFile=meta, by=0.05,prefix=prefix){
    ...
    for(ind in 1:ncol(CPM)) {
        ...
        for(K in SEQ) {    
        counter <- counter+1
           if( counter%%100 ==0 | counter== 1){ cat(paste0(counter,".."))}
...
CalculateSF <- function(data=data,metaFile=meta, prefix=prefix, dataRange=c(0.1, 0.99)){
...
#' This function generates boxplot using sacaling factor table. It's been included in the last step of ChIPseqSpikeFree().
#' 
#' @param input a file/data.frame generated/returned by CalculateSF() or ChIPseqSpikeFree(). 
...
#' @export
#' @examples
#' ##1. re-generate boxplot of ChIPseqSpikeFree scaling factors
...
BoxplotSF <- function(input=meta.SF,prefix=prefix){
   ...
   if (! "COLOR" %in% colnames(input)){
    ...
    myTitle <- "ScalingFactor~GROUP+ANTIBODY"
        bp <- boxplot( SF~GROUP+ANTIBODY,  data=input ,main=myTitle,
       ...
       stripchart( SF~GROUP+ANTIBODY, data=input[!input$SF %in% bp$out,], vertical = T, at=usedXs,
...
ChIPseqSpikeFree <- function(bamFiles=bams, chromFile=chrom.size, metaFile=metaFile,prefix=prefix){
